...
public class BlogValidation {
    public static boolean isValidUser(Blog blog) throws ValidationException {
        ...
        if(blog.getBlogName().length()<20){
            throw new ValidationException("Blog name should be more then 20 characters");
        }
        if(blog.getCategory().length()<20){
            throw new ValidationException("Blog category should be more then 20 characters");
        }
